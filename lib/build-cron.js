...
module.exports = function buildCronJobs(cronConfigsDir, collectionName, logCleanUpOpts) {
  ...
  if (!cronConfigsDir || !collectionName) {
    throw new VError('Error: \'cronConfigsDir\' and \'collectionName\' are required params.');
  }
  ...
  const instance = {
    ...
    loadJobs: () => {
      cmaster.loadJobs(cronConfigsDir, function (err, jobs) {
        if (err) {
          ...
          throw new VError('No jobs found in directory: %s', cronConfigsDir);
        } else {
    ...
    getHandlers: (job) => {
      if (!job) {
        throw new VError('getHandlers must be passed a cron-master job instance');
      }
