...
function initialiseJob(job, db) {
  ...
  job.on(cmaster.EVENTS.TICK_STARTED, function (err, result, time) {
    if (err) {
      //mongo collection DEBRIEFS_CRON_LOGS
    ...
    .then(function(res) {
      //storing the collection id in job key
      job.meta.id = res.insertedId;
  ...
  job.on(cmaster.EVENTS.TICK_COMPLETE, function (err, result, time) {
    ...
    return db.then(function(coll) {
      return coll.updateOne({_id: job.meta.id}, {$set: data}, false);
    });
  });
...
function createLogCleanUpJob(logCleanUpOpts, db) {
  ...
  // Delete logs older than logRetentionDate
  var cleanUpProcess = {
    run: function(done) {
      var logRetentionDate = moment().subtract(logRetentionPeriod, 's').toDate();
      return db.then(function(coll) {
        return coll.remove({'startTime': {$lt: logRetentionDate}});
      })
      .asCallback(done);
    }
  };
