...
var log = require('tke-logger').getLogger(require('../package.json').name);
 ...
 */
module.exports = function buildCronJobs(cronConfigsDir, collection, logCleanUpOpts) {
  ...
  cmaster.loadJobs(cronConfigsDir, function (err, jobs) {
    if (err) {
      ...
      var logCleanUpJob = createLogCleanUpJob(logCleanUpOpts.frequency, logCleanUpOpts.retentionPeriod, db);
 ...
 */
function createLogCleanUpJob(frequency, retentionPeriod, db) {

  //Default to run every night at midnight
  frequency = frequency || '* * 0 * * *';
  retentionPeriod = retentionPeriod || '7 days';

  var logRetentionPeriod = timestring(retentionPeriod);
