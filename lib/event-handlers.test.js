...
describe('event-handlers', () => {
  let mockJob, mongo, _sut, requires;
  ...
  beforeEach(() => {
    ...
    requires = {
      ['rhmap-raygun-nodejs']: sinon.stub().returns({
    ...
    it('should fail to write the job to mongo and remove an existing _id', () => {
      ...
      return handlers.onTickStarted()
        .then(() => {
          ...
          expect(requires['rhmap-raygun-nodejs']().send.callCount).to.eql(1);
          expect(requires['rhmap-raygun-nodejs']().send.getCall(0).args[0]).to.be.an('error');
        });
    });
  });
